...
const PostItem = props => {
    const {auth, addLike, removeLike, deletePost, post: {_id, text, name, avatar, user, like, comments, date}, showAction} = props;
    return (<div className="post bg-white p-1 my-1">
        <div>
            <Link to={`/profile/${user}`}>
                <img className="round-img" alt="avatr" src={avatar}/>
                <h4>{name}</h4>
            </Link>
        </div>
        <div>
