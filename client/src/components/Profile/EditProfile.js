...
const EditProfile = (props) => {
  let { createProfile, getCurrentProfile, profile:{profile, loading}, history } = props;
  const [formData, setFormData] = useState({company: '',website: '',location: '',
      status:'',skills:'',bio: '',githubusername:'',youtube: '',twitter: '',
      facebook: '',linkedin:'',instagram:'' });
  const [isToggleSocialInp, ToggleSocialInp] = useState(false);
  useEffect(()=>{
    getCurrentProfile();
    setFormData({
  ...
  const {company,website,location,status,skills,bio,githubusername,
      youtube,twitter,facebook,linkedin,instagram} = formData;
  ...
  return(<Fragment>
    ...
    <form className="form" onSubmit={e=>onSubmit(e)}>
      <div className="form-group">
        ...
        <div className="my-2">
          <button type="button" className="btn btn-light" onClick={()=>ToggleSocialInp(!isToggleSocialInp)}>
            Add Social Network Links
          </button>
          <span>Optional</span>
        </div>
        {
          isToggleSocialInp && <Fragment>
